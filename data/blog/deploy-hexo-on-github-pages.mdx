...
summary: 'Hexo是一个快速、简洁的静态博客框架。安装Hexo需要先安装Node.js和Git，并使用npm安装Hexo-cli。创建博客后，可以通过 `hexo init` 命令初始化博客目录并使用 `hexo server` 启动本地服务器。在部署过程中，需在GitHub创建个人访问令牌并配置 `_config.yml` 文件，之后安装 `hexo-deployer-git` 插件，使用 `hexo clean && hexo deploy` 部署到GitHub Pages。可以通过Gitalk实现评论功能，还可以设置自定义域名来访问博客。'
authors: ['default']
---
